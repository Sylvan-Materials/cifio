...
TEST_CASE("test 3sgs") {
   ...
   CHECK_EQ(lemon::countEdges(graph.componentGraph), 4);
    ...
    for(lemon::ListGraph::NodeIt nComponentA(graph.componentGraph); nComponentA!=lemon::INVALID; ++nComponentA){
        if(graph.componentProperties[nComponentA].termination==sylvanmats::constitution::MONOMER)cifPublisher.insertNonpolymers(std::make_tuple(graph.componentProperties[nComponentA].asym_id, graph.componentProperties[nComponentA].entity_id, graph.componentProperties[nComponentA].mon_id, graph.componentProperties[nComponentA].ndb_seq_num, graph.componentProperties[nComponentA].pdb_seq_num, graph.componentProperties[nComponentA].auth_seq_num, graph.componentProperties[nComponentA].pdb_mon_id, graph.componentProperties[nComponentA].auth_mon_id, graph.componentProperties[nComponentA].pdb_strand_id, graph.componentProperties[nComponentA].pdb_ins_code));
