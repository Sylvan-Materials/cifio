...
all: LDFLAGS= -shared  -fuse-ld=gold -L../lemon-main/dist/lib -L$(JAVA_HOME)/lib -L$(LUA_HOME)/install/lib -L$(JAVA_HOME)/lib/server -Lcpp_modules/urlcpp -L../antlr4/runtime/Cpp/run/usr/local/lib -Lcpp_modules/zlib/dist/lib -Lcpp_modules/openssl/openssl/lib64 -ljvm -llua -lantlr4-runtime -lemon -lurlcpp -lz -ldl -pthread -lssl -lcrypto
...
build/src/reading/tcp/TCPReader.o: CXXFLAGS= -DNDEBUG -O3 -fPIC -pedantic -pthread -std=c++23 -Iinclude -Isrc -I../lemon-main/dist/include -Icpp_modules/urlcpp -Icpp_modules/openssl/openssl/include -Icpp_modules/zlib/dist/include -I../antlr4/runtime/Cpp/run/usr/local/include/antlr4-runtime -MMD
